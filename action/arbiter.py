...
class Arbiter(ABC):
    ...
    Арбитер полностю управляет взаимодействием клеток друг с другом и он выставляет внутренние состояния клеток.
    """
...
class HerbArbiter(Arbiter):
    ...
    def cell_move(
        ...
        start.set_next(move)
        move.set_next(eat)
        eat.set_next(reproduction)

        can_processing = start.handle(cell, coordinates, old_map, new_map)

        if not can_processing:
            x, y = coordinates
            new_map[y][x] = cell
